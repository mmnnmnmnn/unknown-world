...
/* =======================================================
   ...
   3.9~7.0s   : 물고기 떼 B 우 → 좌 (좌우 반전)
   7.35~11.5s : 고래 좌 → 우, 위아래로 천천히 부유
   10.0~14.5s : 카메라 다시 상승
   10.15~13.2s: 대왕오징어 우 → 좌, 이동하며 축소
   13.2~14.5s : 좌우 반전 후 우상단 빠른 퇴장
   14.9~15.6s : "닿을 수 없는 심해," blur → sharp
   15.6~19.0s : 문구 유지 (Scene 5 직전)
======================================================= */
...
const SCENE4_TIMELINE = {
  ...
  squidStart: 10150,
  squidApproachEnd: 13200,
  squidExitEnd: 14500,

  titleStart: 14900,
  titleReady: 15600,

  preScene5HoldEnd: 19000
};
...
function renderScene23(time) {
  ...
  const closeupZoom = lerp(1.48, 1.10, closeupFadeIn);
  let closeupRotation = 0;

  /* -----------------------------------------------------
     4. Scene 2 → Scene 3 Match
     A) 11.2~11.95초: 0.75초 동안 천천히 회전
     B) 11.95초 이후: 회전 정지
     C) 11.95~13.95초: 회전한 상태 그대로 천천히 fade out
  ----------------------------------------------------- */
...
function renderWhale(time) {
  ...
  const x = lerp(-58, 126, p);
  const bob = Math.sin(
    (time - SCENE4_TIMELINE.whaleStart) / 330
  ) * 1.55;
  ...
  const fadeIn = easeSmooth(
    segmentProgress(
      time,
      SCENE4_TIMELINE.whaleStart,
      SCENE4_TIMELINE.whaleStart + 520
    )
  );
  const fadeOut = 1 - easeSmooth(
    segmentProgress(
      time,
      SCENE4_TIMELINE.whaleEnd - 520,
      SCENE4_TIMELINE.whaleEnd
    )
  );
...
function renderSquid(time) {
  ...
  const bob = Math.sin(
    (time - SCENE4_TIMELINE.squidStart) / 260
  ) * 1.35;
  ...
  if (time <= SCENE4_TIMELINE.squidApproachEnd) {
    ...
    const fadeIn = easeSmooth(
      segmentProgress(
        time,
        SCENE4_TIMELINE.squidStart,
        SCENE4_TIMELINE.squidStart + 420
      )
    );

    // 우측에서 등장 → 왼쪽으로 이동하면서 점점 작아짐.
  ...
  // 중앙 부근에서 즉시 좌우 반전한 뒤 우상단으로 빠르게 퇴장.
  ...
  const fadeOut = 1 - easeSmooth(
    segmentProgress(
      time,
      SCENE4_TIMELINE.squidExitEnd - 380,
      SCENE4_TIMELINE.squidExitEnd
    )
  );
  ...
  setScene4Creature(scene4Squid, {
    x: lerp(47, 124, exitP),
    y: lerp(46, -12, exitP) + bob,
    scale: lerp(0.58, 0.34, exitP),

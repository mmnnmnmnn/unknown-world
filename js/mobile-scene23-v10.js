...
const SCENE23_TIMELINE = {
  ...
  closeupRotateEnd: 11950,

  // 회전이 멈춘 뒤 더 천천히 사라짐
  closeupFadeOutStart: 11950,
  closeupFadeOutEnd: 13950,

  // 은하단 이미지는 같은 위치에서 아래에 나타나며 match
  clusterFadeStart: 11200,
  clusterFadeEnd: 13950,

  // closeup이 완전히 사라진 뒤에만 Zoom Out 시작
  clusterZoomOutStart: 13950,

  // 줌아웃 시작 후 0.5초 뒤 중앙 이동 시작
  clusterPanStart: 14450,
  clusterZoomOutEnd: 17750,

  // Scene 3 완료 텍스트
  scene3TitleStart: 18100,
  scene3TitleReady: 18850,
...
function resetScene23() {
  ...
  scene3EndTitle.style.transform = "translateX(-50%) translateY(10px)";
}
...
function renderScene23(time) {
  ...
  scene3EndTitle.style.transform =
    `translateX(-50%) translateY(${lerp(10, 0, titleP)}px)`;
}
...
function scene23Loop(now, token) {
  ...
  if (elapsed >= SCENE23_END) {
    ...
    // 현재 개발 단계에서는 Scene 4 직전 장면까지 보여준 후
    // 임시로 질문 입력 화면으로 이동.
    renderScene14();
    return;
  }

...
const SCENE23_TIMELINE = {
  scene1ToMilkywayEnd: 900,

  milkywayHoldEnd: 3900,
  milkywayZoomEnd: 7000,

  closeupFadeStart: 5200,
  closeupReady: 7000,
  closeupHoldEnd: 8400,

  clusterMatchStart: 8400,
  clusterMatchEnd: 9400,

  clusterZoomOutStart: 9000,
  clusterZoomOutEnd: 12600,

  scene3ArrivalHoldEnd: 14600
};

const MILKYWAY_TARGET = {
  // 은하수 안의 밝은 밀집 영역을 1차 확대 중심으로 설정
  x: 47.5,
  y: 48.0
};
...
------------------------------------------------------- */
function scene23Matrix(targetX, targetY, zoom, rotation = 0) {
  ...
  return (
    ...
    `translate(${-targetPxX}px, ${-targetPxY}px)`
  );
}
...
function resetScene23() {
  ...
  milkywayLayer.style.transform = scene23Matrix(50, 50, 1);
...
function renderScene23(time) {
  ...
  /* -----------------------------------------------------
     3. Closeup galaxy
     은하수 확대 후 고해상도 은하 이미지로 Crossfade
  ----------------------------------------------------- */
  let closeupOpacity = closeupFade;
  let closeupZoom = lerp(1.55, 1.12, closeupFade);
  let closeupRotation = 0;

  /* -----------------------------------------------------
     4. Scene 2 → Scene 3 Match Dissolve
     closeup galaxy가 회전/축소하면서
     cluster의 지정 은하 위치와 연결
  ----------------------------------------------------- */
  const match = scene23Ease(
    time,
    SCENE23_TIMELINE.clusterMatchStart,
    SCENE23_TIMELINE.clusterMatchEnd
  );

  if (time >= SCENE23_TIMELINE.clusterMatchStart) {
    closeupOpacity = 1 - match;
    closeupZoom = lerp(1.12, 0.62, match);

    // closeup의 대각선 은하를 cluster 타깃의 세로형 장축에 가깝게 회전
    closeupRotation = lerp(0, 48, match);
  }
  ...
  /* -----------------------------------------------------
     5. Cluster
     타깃 은하 (64.03, 31.92)를 화면 중앙에 둔 채
     약 14배 → 1배로 Zoom Out
  ----------------------------------------------------- */
  ...
  const clusterZoom = lerp(14, 1, zoomOut);
  const clusterX = lerp(CLUSTER_TARGET.x, 50, zoomOut);
  const clusterY = lerp(CLUSTER_TARGET.y, 50, zoomOut);
...
function scene23Loop(now, token) {
  ...
  // Crossfade가 끝나면 Scene 1은 실제로 숨겨도 됨.
  if (
